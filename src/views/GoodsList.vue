  ...
  <div>
    ...
    <model v-bind:mdShow="isShowAddCartSuccess" @closeModel="closeAddCartSuccess">
      ...
      <div slot="btn">
        ...
        <router-link class="btn btn--m btn--red" href="javascript:;" to="/cart">购物车</router-link>
      </div>
    </model>
    <model v-bind:mdShow="isShowMessage" @closeModel="isShowMessage=false">
      <div slot="content">
        <p>登录用户名：admin</p>
        <p>登录密码：123456</p>  
      </div>
      <div slot="btn">
        <a class="btn btn--m" href="javascript:;" @click="isShowMessage = false">确定</a>
      </div>
    </model>
...
export default {
  data() {
    return {
      ...
      loading: true, //控制无限滚动加载的图标的显示
      startPrice: "0", //传到后端的数据：价格帅选功能
      endPrice: "10000",
      isShowAddCartFail: false, //加入购物车失败的弹窗显示:用户未登录的情况下
  ...
  methods: {
    getGoodsList(flag) {
      ...
      this.loading = true; //在等待时，无限滚动加载图标出现
      axios //发送axios请求，注意post传参时get传参格式不一样，post 多了一个params
        .get("/goods/list", {
          params: params
        })
        .then(res => {
          this.loading = false; //获得数据后，无限滚动加载图标（其实就是正在加载的图标）消失
          if (flag) {
            //flag用来用控制是  第一次加载，还是分页的加载（分页的加载，前面有数据了）
            this.goodsList = this.goodsList.concat(res.data.result.list);
            //不拼接前面的数据，则会出现，分页的时候，只会显示新的数据，前面的数据会消失
            if (res.data.result.count === 0) {
              //当传回来的数据小于8条时，把busy为true，不再继续懒加载
              this.busy = true;
    ...
    loadMore() {
      /*
      loadMore 就是控制无限加载的函数，无限滚动加载函数的启动是，当你滚动距离
      屏幕底部距离为30（这是自己设定的），就会触发loadMore函数，loadMore函数中，加busy置为true，
      无限滚动加载功能可以触发，随后，发送请求
      */
      this.busy = true; //这个是控制 无限滚动加载 是否 可以触发
      setTimeout(() => {
        this.page++;
